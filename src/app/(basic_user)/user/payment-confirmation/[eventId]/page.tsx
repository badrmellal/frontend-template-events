...
import { ArrowLeft, CreditCard, Clock, Calendar, MapPin, AlertTriangle, Lock } from "lucide-react"
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/tickets/purchase/${event.id}`,
        {
      ...
      if (response.status === 200) {
        if (event.isFreeEvent) {
          ...
          // #TODO
          // For paid events, we need handle payment confirmation later
  ...
  if (error || !event || !booking) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-black text-white">
        <p className="text-red-500 px-8">{error || "Event or booking details not found"}</p>
        <Button className='bg-white my-5 text-black hover:bg-black hover:text-white' onClick={handleErrorButton}>
          Home
        </Button>
      </div>
    )
    ...
    <Dialog open={isExpired} onOpenChange={setIsExpired}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center text-yellow-400">
          ...
          >
            <Lock className='mr-2'/>
            {isExpired ? 'Booking Expired' : (event.isFreeEvent ? 'Confirm Booking' : 'Proceed to Payment')}
          </Button>
        </div>
      </div>
    </div>
